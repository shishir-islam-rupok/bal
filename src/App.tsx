...
import { convertMediaFile, MEDIA_FORMAT_OPTIONS, type MediaOutputFormat, validateMediaFile } from './lib/mediaConverter'
import './App.css'
...
function App() {
  ...
  async function convertAll() {
    ...
    for (const item of pendingFiles) {
      try {
        const blob = mode === 'image' ? await convertFile(item.file, format) : await convertMediaFile(item.file, mediaFormat)
    ...
    <div className="app-shell">
        ...
        {view === 'converter' && <>
          <section className="converter-hero">
            <div className="intro">
              ...
              {mode === 'image' ? <><div className="format-chips" aria-label="Choose output format"><button className={format === 'jpg' ? 'selected' : ''} type="button" onClick={() => setFormat('jpg')}>JPG</button><i>to</i><button className={format === 'png' ? 'selected' : ''} type="button" onClick={() => setFormat('png')}>PNG</button><button className={format === 'webp' ? 'selected' : ''} type="button" onClick={() => setFormat('webp')}>WEBP</button></div><p className="format-note">Inputs: JPG, JPEG, PNG, WEBP, GIF, BMP, and AVIF</p></> : <div className="format-chips" aria-label="Choose output format">{Object.entries(MEDIA_FORMAT_OPTIONS).map(([value, option]) => <button key={value} className={mediaFormat === value ? 'selected' : ''} type="button" onClick={() => setMediaFormat(value as MediaOutputFormat)}>{option.extension.toUpperCase()}</button>)}</div>}
              <div className="steps"><div><b>1</b><span>Choose images</span></div><div><b>2</b><span>Pick a format</span></div><div><b>3</b><span>Download</span></div></div>
            </div>
            ...
            <section className="workspace" aria-label="Image converter">
              ...
              >
                ...
                <small>{mode === 'image' ? 'JPG, PNG, WEBP, GIF, BMP, or AVIF · up to 20 MB each' : 'MP4, MKV, MOV, MP3, WAV, OGG, or M4A · up to 100 MB each'}</small>
              </div>
              ...
              {files.length > 0 && <div className="file-list" aria-live="polite">
                {files.map((item) => <div className="file-row" key={item.id}>
                  <div className="file-preview"><img src={item.previewUrl} alt="" /></div>
          ...
          <section className="content-section converter-guide">
      ...
      <footer className="site-footer">
        <div className="footer-main">
          ...
          <div className="footer-column"><strong>Information</strong><button type="button" onClick={() => setView('privacy')}>Privacy policy</button><button type="button" onClick={() => setView('terms')}>Terms of use</button><button type="button" onClick={() => setView('contact')}>Contact support</button></div>
        </div>
