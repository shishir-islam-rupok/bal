        ...
        {view === 'converter' && (
          <>
            <section className="converter-hero">
              <div className="intro">
                <div className="tool-switcher" role="group" aria-label="Choose converter type"><button aria-pressed={mode === 'image'} className={mode === 'image' ? 'selected' : ''} type="button" onClick={() => { clearFiles(); setMode('image') }}>Images</button><button aria-pressed={mode === 'media'} className={mode === 'media' ? 'selected' : ''} type="button" onClick={() => { clearFiles(); setMode('media') }}>Audio &amp; video</button></div>
                ...
                {mode === 'image' ? <><div className="format-chips" role="group" aria-label="Choose output format">{Object.entries(FORMAT_OPTIONS).map(([value, option], index) => <Fragment key={value}>{index > 0 && <i>·</i>}<button aria-pressed={format === value} className={format === value ? 'selected' : ''} type="button" onClick={() => setFormat(value as OutputFormat)}>{option.extension.toUpperCase()}</button></Fragment>)}</div><p className="format-note">Inputs: JPG, JPEG, PNG, WEBP, GIF, BMP, and AVIF</p></> : <div className="format-chips" role="group" aria-label="Choose output format">{Object.entries(MEDIA_FORMAT_OPTIONS).map(([value, option]) => <button aria-pressed={mediaFormat === value} key={value} className={mediaFormat === value ? 'selected' : ''} type="button" onClick={() => setMediaFormat(value as MediaOutputFormat)}>{option.extension.toUpperCase()}</button>)}</div>}
